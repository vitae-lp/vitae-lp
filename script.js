...
products.forEach(button => {
    button.addEventListener("click", () => {
        ...
        const variantsData = card.dataset.variants; // Lee el texto con las opciones
        ...
        viewDescription.innerHTML = card.dataset.description;

        // LÓGICA DINÁMICA DE VARIANTES
        if (variantsData) {
            variantContainer.style.display = "block";
            variantSelect.innerHTML = ""; // Limpiamos

            // Separamos las opciones (usamos split para romper el texto)
            const options = variantsData.split(","); 
            options.forEach(optText => {
                const [nombre, precio, img] = optText.trim().split(":");
            ...
            variantSelect.onchange = () => {
                ...
                viewPrice.textContent = `$${parseInt(selected.value).toLocaleString('es-AR')}`;
                viewImage.src = selected.dataset.img;
            };
        } else {
...
document.getElementById("btn-confirm-wsp").onclick = function() {
    ...
    const mensajeWSP = `¡Hola VITAE! %0A%0A` +
        ...
        `*Detalles del pedido:*%0A${listaProductos}%0A` +
...
if (addToCartBtn) {
    addToCartBtn.onclick = function() {
        const variantSelect = document.getElementById("variantSelect");
        const isVariantVisible = document.getElementById("variant-container").style.display === "block";

        let finalName = viewTitle.textContent;
        let finalPrice = parseFloat(viewPrice.textContent.replace("$", "").replace(/\./g, "").trim());
        let finalImage = viewImage.src;

        if (isVariantVisible) {
            const selected = variantSelect.options[variantSelect.selectedIndex];
            finalName = `${viewTitle.textContent} (${selected.textContent})`;
        }

        cart.push({ name: finalName, price: finalPrice, image: finalImage });
        updateCart();
        saveCart();
        updateCartCounter();
        productView.style.display = "none"; // Cerramos el visor
